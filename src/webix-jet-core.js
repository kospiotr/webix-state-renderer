...
function url_string(stack) {
  var url = [];
  var start = webixJet.config.layout ? 1 : 0;
...
function parse_parts(url) {
  ...
  if (!chunks[0]) {
    if (webixJet.config.layout)
      chunks[0] = webixJet.config.layout;
    else
      chunks.shift();
  }
...
var webixJet = {
  create: function (config) {
    ...
    //init self
    webixJet.debug = config.debug;
    webixJet.$layout = {
      sub: subui,
      root: webixJet.config.container,
      index: 0,
      add: true
    };
    webix.extend(webixJet, webix.EventSystem);
    ...
    var node = webixJet.config.container;
  ...
  router: function (name) {
    ...
    webixJet.path = [].concat(parts);

    //webix.ui.$freeze = true;
    render_stack(webixJet.$layout, parts);
  },
  ...
  trigger: function (name) {
    webixJet.apply(name, [].splice.call(arguments, 1));
  },
  apply: function (name, data) {
    webixJet.callEvent(name, data);
  },
  action: function (name) {
    return function () {
      webixJet.apply(name, arguments);
    };
  },
